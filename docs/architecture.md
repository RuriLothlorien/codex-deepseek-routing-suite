...
- 模型识别保护：`routerModelFor` 仅对显式 Pro 标记（`pro|reasoner|r1|v4-pro` 且非 flash）走 Pro 分支；模型缺失/未识别（如 `deepseek-chat`、空值）一律回退已实测的 `deepseek-v4-flash`，防止误用未测试的 Pro persona。
