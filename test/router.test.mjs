...
test('spec: planning/porting tasks map to spec band (方案A)', () => {
  assert.equal(classifyTask('阅读 /tmp/legacy-project，规划一个在我的Codex上完整实现的最佳移植方案'), 0)
  assert.equal(bandFor(classifyTask('帮我写一个移植方案')), 'spec')
  assert.equal(bandFor(classifyTask('先阅读代码库，然后规划迁移计划')), 'spec')
  assert.equal(bandFor(classifyTask('write a survey of the codebase')), 'spec')
  assert.equal(bandFor(classifyTask('plan the migration')), 'spec')
})
